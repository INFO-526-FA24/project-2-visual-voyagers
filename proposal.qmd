...
#####Approach:
-	We will be using three models: SVM, Logistic Regression, Naive Baye's. 
-	After training the model with the dataset, we will compare and analyze the the accuracy of the model’s performance.
...
#####Approach: We will be using three models: SVM ,LR and NB.
-	After training the model with the dataset, we will compare the ROC and the AUC which represents the probability of that the model, if given a randomly chosen positive and negative example, will rank the positive higher than the negative.
...
**Question 4**: 
#####Approach: 
-       Use the correlation matrix to identify the two most strongly correlated features with the target variable for each model.
-       Train each model (SVM, LR, NB) using all features.
-       Visualize the decision boundaries and classification regions based only on the top two features, while keeping the model trained on all features.
...
-      Plot the data points on a 2D plane with the top two features on the axes.
-      Overlay the decision boundaries and classification regions for each model.
-      Use distinct colors to represent the predicted classes and markers to show actual class labels.
